...
export interface SpiaggiaSeed {
  ...
  lat: number;
  lng: number;
  // Campi opzionali arricchiti — popolati solo dove abbiamo dati verificati
  stagioneInizio?: string; // formato "DD/MM" (es. "19/04")
  stagioneFine?: string; // formato "DD/MM" (es. "02/11")
  orari?: string; // es. "6:00-20:00" oppure "24h"
  guinzaglio?: "obbligatorio" | "libero" | "solo-fuori-acqua";
  bagnoConsentito?: boolean;
  prezzoTipo?: "gratuito" | "pagamento" | "con-consumazione";
  parcheggio?: "gratuito" | "pagamento" | "assente";
  telefono?: string;
  website?: string;
  instagram?: string;
  ordinanzaUrl?: string;
...
export function slugifyRegione(r: string): string {
  return r
    .toLowerCase()
    .replace(/'/g, "")
    .replace(/\s+/g, "-");
...
export function unslugifyRegione(slug: string): string | null {
  const match = Array.from(new Set(SPIAGGE_SEED.map((s) => s.regione))).find(
    (r) => slugifyRegione(r) === slug
  );
  return match || null;
}

export function getSpiaggeByRegione(regioneSlug: string): SpiaggiaSeed[] {
  return SPIAGGE_SEED.filter((s) => slugifyRegione(s.regione) === regioneSlug);
}

export function getSpiaggiaBySlug(slug: string): SpiaggiaSeed | undefined {
...
export const SPIAGGE_SEED: SpiaggiaSeed[] = [
  ...
  { slug: "spiaggia-libera-lido-di-classe", nome: "Spiaggia Libera Cani - Lido di Classe", comune: "Lido di Classe", provincia: "RA", regione: "Emilia-Romagna", tipo: "Spiaggia libera", descrizione: "Tratto di spiaggia libera dedicato ai cani. Accesso dal 19 aprile al 2 novembre, ore 6-20. Regolamentata dal Comune di Ravenna.", servizi: ["Ingresso libero", "Accesso regolamentato", "Tratto dedicato"], img: "https://images.unsplash.com/photo-1507146426996-ef05306b995a?w=400&q=80", lat: 44.3091, lng: 12.3452, stagioneInizio: "19/04", stagioneFine: "02/11", orari: "6:00-20:00", guinzaglio: "solo-fuori-acqua", bagnoConsentito: true, prezzoTipo: "gratuito", parcheggio: "gratuito" },
  ...
  // === TOSCANA ===
  { slug: "dog-beach-san-vincenzo", nome: "Dog Beach San Vincenzo", comune: "San Vincenzo", provincia: "LI", regione: "Toscana", tipo: "Spiaggia libera", descrizione: "200 metri di battigia, parco ombreggiato, area recintata, docce per cani. I cani possono stare senza guinzaglio se educati. Tra le piu famose dog beach toscane.", servizi: ["200m battigia", "Parco ombreggiato", "Area recintata", "Docce", "Cani senza guinzaglio"], img: "https://images.unsplash.com/photo-1601758174114-e711c0cbaa69?w=400&q=80", lat: 43.0639, lng: 10.5343, stagioneInizio: "01/05", stagioneFine: "30/09", guinzaglio: "libero", bagnoConsentito: true, prezzoTipo: "gratuito", parcheggio: "pagamento" },
